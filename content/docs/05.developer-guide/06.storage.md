...
The [PurgeExecution](../../plugins/core/tasks/storages/io.kestra.core.tasks.storages.PurgeExecution.md) task can purge all the files stored inside the internal storage by a flow execution.
...
You can also use the [Purge](../../plugins/core/tasks/storages/io.kestra.core.tasks.storages.Purge.md) task to purge executions, logs, metrics, and internal storage files of past executions. 

For example, this flow will purge all of these every day for executions older than one month:
```yaml
id: purge
namespace: io.kestra.tests
